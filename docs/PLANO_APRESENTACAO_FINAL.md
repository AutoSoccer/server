...
- **Entregavel:**
  ...
  - UptimeRobot configurado para fazer ping em `https://autosoccer.onrender.com/health` a cada 5 min
  - Screenshot dos dashboards Sonar + UptimeRobot no `server/docs/monitoring.md`
  - **Atencao:** depende de deploy ativo no Render (item urgente)
- **Criterio de aceite:**
...
- **Entregavel:**
  ...
  - Render Blueprint com 2 services: `autosoccer-staging` e `autosoccer-production`
  - Banco MySQL separado por ambiente
- **Criterio de aceite:**
...
- **Entregavel:**
  - README de ambos repos com:
    - Badge Sonar
    - URL Render publica (production e staging)
    - URL Swagger publica
...
| 26 | Deploy Render + .env por ambiente + secrets sync:false | Lucas B | 0:40 | ID2.4 |
...
|---|---|---|---|
| Render down no dia da apresentacao | media | alto | Demo local no laptop + screenshots backup + video gravado |
...
- [ ] **Deploy Render** — NAO ESTA ATIVO. Bloqueia T8 (Sonar + UptimeRobot). Acao: Lucas Bruno cria conta + provisiona MySQL externo (Railway) + conecta repos via Blueprint
...
### Guia rapido para subir o deploy no Render

1. Lucas Bruno: criar conta em https://render.com (login com GitHub recomendado)
2. Provisionar MySQL externo gratuito em Railway (https://railway.app) — Render free tier nao oferece MySQL
3. No Render, escolher "New > Blueprint" e apontar para o repo `server` (ja tem `render.yaml` configurado)
4. Definir secrets manualmente no painel do Render:
   - `DATABASE_URL` = string de conexao do Railway
   - `JWT_SECRET` = qualquer string aleatoria longa
   - `CORS_ORIGIN` = URL do front (sera definida no proximo passo)
5. Aguardar build do server, copiar a URL publica (algo como `https://autosoccer-api.onrender.com`)
6. Criar novo Static Site ou Web Service para o `front` no Render
   - Build command: `npm install && npm run build`
   - Start command: `npm start`
   - Env var: `NEXT_PUBLIC_API_URL` = URL do server do passo anterior
7. Voltar no server e atualizar `CORS_ORIGIN` com a URL do front
8. Smoke test: abrir `https://<server>/health` e `https://<server>/docs`, testar login no front
9. Configurar UptimeRobot fazendo ping em `/health` a cada 5 min
